from pydantic import BaseModel, EmailStr, Field, ConfigDict, field_validator # Use ConfigDict for Pydantic v2
...
class TicketBase(BaseModel):
    """Base schema for ticket properties."""
    ...
    # --- assigned_to is now Optional[int], allowing None ---
    assigned_to: Optional[int] = Field(None, description="ID of the user assigned to handle the ticket. Set to null or omit to leave unassigned.", validate_default=True)
    # --- End Change ---
    ...
    comment_log: Optional[str] = Field(None, description="A simple log of comments (consider separate Comment model)")
    subtask: Optional[str] = Field(None, description="Details about any subtasks related to this ticket")
    credit: Optional[int] = Field(1, description="Workload credit value associated with the ticket")

    model_config = ConfigDict(
        use_enum_values=True, # Ensure enums serialize to values
    )

class TicketCreate(TicketBase):
    """Schema for creating a new ticket. Inherits from TicketBase."""
    # assigned_to is optional by default from TicketBase.
    # If you *require* an assignee on creation, uncomment the next line:
    # assigned_to: int = Field(..., description="ID of the user assigned to handle the ticket")
    pass

class TicketUpdate(BaseModel):
    """
    Schema for updating an existing ticket. All fields are optional.
    Allows setting assigned_to to null to unassign.
    """
    ...
    # --- assigned_to is Optional[int], allowing null for unassignment ---
    # The 'None' default and Optional type implicitly handle this.
    # Added explicit description note.
    assigned_to: Optional[int] = Field(None, description="ID of the user assigned to handle the ticket. Set to null to unassign.")
    # --- End Change ---
    ...
    subtask: Optional[str] = Field(None, description="Details about any subtasks related to this ticket")
    credit: Optional[int] = Field(None, description="Workload credit value associated with the ticket")
...
class Ticket(TicketBase):
    """Schema representing a ticket retrieved from the database, including joined data."""
    ...
    # --- assigned_to_name is Optional to match assigned_to ---
    ...
    model_config = ConfigDict(
        from_attributes=True, # Enable ORM mode compatibility
        use_enum_values=True,
         json_schema_extra = { # Example remains largely the same, assigned_to can be null
            "example": {
                "id": 1,
                "room_id": 102,
                ...
                "status": "Open",
                "priority": "High",
                "assigned_to": 2, # Example: Assigned
                # OR "assigned_to": null, // Example: Unassigned
                "created_by": 1,
                ...
                "room_number": "102",
                "assigned_to_name": "Bob Housekeeper", # Example: Assigned
                # OR "assigned_to_name": null, // Example: Unassigned
...
class BatchUpdateRequest(BaseModel):
    """
    Defines the structure for requesting batch updates on entities.
    Used by confirmation endpoints.
    """
    ids: List[int] = Field(..., min_length=1, description="A list of entity IDs to update.")
    payload: Dict[str, Any] = Field(..., min_length=1, description="A dictionary containing the field-value pairs to apply to each entity.")

    @field_validator('ids')
    @classmethod
    def check_ids_are_positive_and_int(cls, v: List[Any]):
        """Validates that all provided IDs are positive integers."""
        validated_ids = []
        for id_ in v:
            if not isinstance(id_, int):
                 try:
                     # Attempt conversion if not int (e.g., numeric string)
                     id_int = int(id_)
                 except (ValueError, TypeError):
                      raise ValueError(f"ID '{id_}' must be an integer.")
            else:
                 id_int = id_

            if id_int <= 0:
                raise ValueError(f"ID '{id_int}' must be a positive integer.")
            validated_ids.append(id_int)
        return validated_ids # Return the list of validated integers

    # Optional: Add example to schema docs
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "ids": [101, 105, 203],
                "payload": {
                    "cleaning_priority": "Low", # Example for rooms
                    "notes": "Batch update applied."
                }
            }
        }
    )
